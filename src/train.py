...
def _instantiate_optimiser(config: Dict[str, Any], compressor: TailWeightedLogisticCompressor):
    algo = config["algorithm"].lower().replace("_", "-").strip()
    if algo in {"boil", "boil-original", "boil-original"}:
        return BOILOptimiser(config, compressor)
    if algo in {"atw-boil", "atw-boil-fixed", "atwboil"}:
        return ATWBOILOptimiser(config, compressor)
    if algo in {"gp-endscore", "gp-end-score"}:
        return GPEndScoreOptimiser(config, compressor)
    if algo in {"hyperband-asha", "hyperbandasha", "asha"}:
        return HyperbandASHAOptimiser(config, compressor)
...
def main():
    ...
    compressor = TailWeightedLogisticCompressor(
        ...
        # alpha is ignored by BOILOptimiser internally unless ATW variant
